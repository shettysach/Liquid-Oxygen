...
interpret statements = fmap fst <$> runExceptT (interpret' statements global)

interpret' ::
  [Stmt] -> Env -> ExceptT RuntimeError IO (Literal, Env)
...
evaluate (Assignment var expr) env = do
  ...
  return $ do
    result >>= \(lit, env') -> case Env.assign (fst var) lit env' of
